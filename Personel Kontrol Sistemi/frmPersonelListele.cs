...
namespace Personel_Kontrol_Sistemi
{
    public partial class frmPersonelListele : Form
    {
        ...
        {
            Database.Listele_Ara(dataGridView1, "select p.PersonelID,p.Adi,p.Soyadi,p.Telefon,p.Adres,p.Email," +
        ...
        private void btnSil_Click(object sender, EventArgs e)
        {
            ...
            string sorgu2 = "update personeller set durumu='Pasif',CikisTarihi=@CikİSTarihi where PersonelID='" + p.PERSONELID + "'";
            SqlCommand komut2 = new SqlCommand();
            komut2.Parameters.Add("@CikisTarihi", SqlDbType.Date).Value = p.CikisTarihi;
            Database.ESG(komut2, sorgu2);
        ...
        private void txtTelefonAra_TextChanged(object sender, EventArgs e)
        {
            Database.Listele_Ara(dataGridView1, "select p.PersonelID,p.Adi,p.Soyadi,p.Telefon,p.Adres,p.Email," +
               "d.Departman, p.Durumu, p.Maasi, p.GirisTarihi, p.Aciklama from Personeller p, Departmanlar d" +
               " where p.DepartmanID = d.DepartmanID and Telefon like '%" + txtTelefonAra.Text + "%'");
        }
